...
echo "$($PSQL "SELECT t1.name AS winner_name FROM games g
...
echo "$($PSQL "SELECT DISTINCT(t1.name) FROM games JOIN teams t1 ON winner_id = t1.team_id  ORDER BY t1.name")"

echo -e "\nYear and team name of all the champions:"
echo "$($PSQL "SELECT year, t1.name FROM games 
JOIN teams t1 ON winner_id = t1.team_id WHERE round='Final' ORDER BY year")"
